...
local local_update_target = function(target)
				...
				if is_valid(t.player) and is_valid(t.player.surface) and r.destroy() then
					...
					if is_player then
						if is_valid(t.player)then
							--e = s.create_entity{name='valkyrie-robot-return-projectile', speed=0.06, position=p, force="player", source=t.player, target=t.player.position}
		...
		if is_valid(r.entity) then
			...
			--local d = max_distance/4
				...
				if enemy ~= nil and #enemy > 0 then 
					...
					elseif true == false then --- only if researched
						local cell = r.logistic_cell
						if cell ~= nil then
							for z = 1, #cell.neighbours do local port = cell.neighbours[z].owner
...
local local_valkyrie_tick = function()		
	--modmash.util.print(#targets .. " " .. #return_targets )
	if global.modmash.valkyries.enable_valkyries ~= true then return end	
	for k = 1, #targets do t = targets[k]
		local_update_target(t)
	end
	if #targets < max_targets then
		local_find_targets()
	end
	for k = #return_targets, 1, -1 do t = return_targets[k]
