...
local generate_surface_area_train = function(x,y,w,h,surface, rock_prefix,force_first_rock)	
	local cx, cy = math.floor(x / 32), math.floor(y / 32)
	if surface.is_chunk_generated({cx, cy}) ~= true then
		surface.request_to_generate_chunks({x, y}, math.max(w,h)*2)
		surface.force_generate_chunk_requests()
	end

	for i = 0,h do
		for j = 0,w do
			local pos = {x = x+j, y = y+i }
			local tile = dirt_prefix .. "dirt-"..math.random(1, 7)
			local current_tile = surface.get_tile(pos)	
...
local local_ensure_can_place_entity_inner = function(entity,event,surface)
	...
	if ename == "subway-level-one" or ename == underground_access or ename == underground_access2 or ename == underground_accumulator then 		
		if surface.level == 0 then			
			...
			if ename == "subway-level-one" then
				...
				--todo implement  check required tracks available or addable
				...
				local rocks = game.surfaces[surface.middle_name].find_entities_filtered{area = {{osd.clear_area.left_top.x, osd.clear_area.left_top.y}, {osd.clear_area.right_bottom.x, osd.clear_area.right_bottom.y}}, name = rock_names}			
				for index=1, #rocks do local r = rocks[index]
					if is_valid(r) then 
						r.destroy({raise_destroy = true}) 
					end			
				...
				generate_surface_area_train(entity.position.x, entity.position.y,osd.width,osd.height ,game.surfaces[surface.middle_name],level_one_rock_prefix)
				return game.surfaces[surface.middle_name].can_place_entity{name="straight-rail", position=osd.track_posistion, direction=sd.track_direction, force=entity.force}  and
					game.surfaces[surface.middle_name].can_place_entity{name=ename, position=sd.opposite_posistion, direction=osd.direction, force=entity.force}	
		...
		elseif entity.surface.name == surface.middle_name then
			...
			elseif ename == "subway-level-one" or  ename == "subway-level-two" then
				--todo implement  check required tracks available or addable
				print("inner can place middle subway-level-one subway-level-two not implemented")
			end
		elseif entity.surface.name == surface.bottom_name then			
			...
			elseif ename == "subway-level-two" then
				...
				generate_surface_area(entity.position.x, entity.position.y,11,game.surfaces[surface.middle_name],resource_level_one,false,level_one_rock_prefix)
				return game.surfaces[surface.middle_name].can_place_entity{name="straight-rail", position=osd.track_posistion, direction=sd.track_direction, force=entity.force}  and
					game.surfaces[surface.middle_name].can_place_entity{name=ename, position=sd.opposite_posistion, direction=osd.direction, force=entity.force}	
...
local local_ensure_can_place_entity = function(entity,event,surface)
	if local_ensure_can_place_entity_inner(entity,event,surface) then			
		...
		if surface.level == 1 and ((etype=="solar-panel" or etype == "rocket-silo") or table_contains(banned_level_one,entity.name)) then		
			...
			return false
		elseif surface.level == 2 and ((entity.name == "subway-level-one" or entity.name == underground_accumulator or entity.name == underground_access or entity.type=="solar-panel" or entity.type == "rocket-silo") or table_contains(banned_level_two,entity.name)) then
			fail_place_entity(entity,event,{"modmashsplinterunderground.underground-placement-disallowed"})
			return false
		elseif surface.level == 0 and (entity.name == "subway-level-two" or entity.name == underground_access2 or table_contains(banned_level_zero,entity.name)) then			
			fail_place_entity(entity,event,{"modmashsplinterunderground.underground-placement-disallowed"})
			return false
		end
	else		
...
local local_underground_added_std = function(entity,event)			
	...
	elseif entity.name == "subway-level-one" then 	
		...
		if surface.level == 0 then
			...
			local sd = local_shape_data(entity.position,entity.direction)
			local u = game.surfaces[surface.middle_name].create_entity{name = entity.name, position = entity.position, force = force_player, direction = sd.opposite_direction}				
			table.insert(stops,{bottom_entity = u, top_entity = entity, added_top_rails = {}, added_bottom_rails = {}})
		elseif surface.level == 1 then
			--todo add tracks if required
			local sd = local_shape_data(entity.position,entity.direction)
			local u = game.surfaces[surface.top_name].create_entity{name = entity.name, position = entity.position, force = force_player, direction = sd.opposite_direction}
			table.insert(stops,{bottom_entity = entity, top_entity = u, added_top_rails = {}, added_bottom_rails = {}})
		end
	elseif entity.name == "subway-level-two" then 	
		...
		if surface.level == 1 then
			...
			local t = game.surfaces[surface.bottom_name].create_entity{name="straight-rail", position=osd.track_posistion, direction=sd.track_direction, force=entity.force}
			local u = game.surfaces[surface.bottom_name].create_entity{name=ename, position=sd.opposite_posistion, direction=osd.direction, force=entity.force}	
			table.insert(stops2,{bottom_entity = u, top_entity = entity, added_top_rails = {}, added_bottom_rails = {t}})
		elseif surface.level == 2 then
			--todo add tracks if required
			local sd = local_shape_data(entity.position,entity.direction)
			local u = game.surfaces[surface.middle_name].create_entity{name = entity.name, position = entity.position, force = force_player,direction = sd.opposite_direction}
			table.insert(stops2,{bottom_entity = entity, top_entity = u, added_top_rails = {}, added_bottom_rails = {}})
		end
	end
	end
...
local local_on_configuration_changed = function(event) 
	...
	if changed then
		if changed.old_version < "1.1.13" then
