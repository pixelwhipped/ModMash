...
local function local_do_teleport(i, p, access, upper_surface, lower_surface)
...
local function equalize_inv(access, flip)
...
local local_ensure_can_place_entity = function(entity,event,surface)
	...
	if local_ensure_can_place_entity_inner(entity,event,surface) then			
		...
		if surface.level == 1 and ((etype=="solar-panel" or etype == "rocket-silo") or table_contains(global.modmashsplinterunderground.banned_level_one,entity.name)) then		
			...
			return false
		elseif surface.level == 2 and ((entity.name == underground_accumulator or entity.name == underground_access or entity.type=="solar-panel" or entity.type == "rocket-silo") or table_contains(global.modmashsplinterunderground.banned_level_two,entity.name)) then
			--print("b")
			fail_place_entity(entity,event,{"modmashsplinterunderground.underground-placement-disallowed"})
			return false
		elseif surface.level == 0 and (entity.name == underground_access2 or table_contains(global.modmashsplinterunderground.banned_level_zero,entity.name)) then			
			--print("a")
			fail_place_entity(entity,event,{"modmashsplinterunderground.underground-placement-disallowed"})
			return false
		end
